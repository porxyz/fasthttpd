...
inline static void MOD_MYSQL_error_log(mysql_connection* conn, const char* message)
{
	...
	SERVER_LOG_WRITE(" MOD MYSQL: ",true);
	SERVER_LOG_WRITE(message,true);
	SERVER_LOG_WRITE("\n",true);
	SERVER_LOG_WRITE(conn->get_last_error(),true);
	SERVER_LOG_WRITE("\n\n",true);
...
inline static void MOD_MYSQL_error_log(mysql_stmt_query* query, const char* message)
{
	...
	SERVER_LOG_WRITE(message,true);
	SERVER_LOG_WRITE("\n",true);
	SERVER_LOG_WRITE(query->get_last_error(),true);
	SERVER_LOG_WRITE("\n\n",true);
	SERVER_LOG_WRITE_ERROR.unlock();
}
...
mysql_connection::mysql_connection() // default constructor
{
	...
	{
		this->enable_error_log = false;
	}
	else
	{
		this->enable_error_log = true;
	}
...
bool mysql_connection::error_log_enabled()
{
	return this->enable_error_log;
}

void mysql_connection::enable_error_logging(bool enable)
{
...
bool mysql_connection::reconnect_if_gone()
{
	if(!this->is_alive())
	{
...
bool mysql_stmt_query::prepare(const std::string& sql,unsigned int bind_params_num,unsigned int bind_result_num)
{
	...
	if((bind_params_num + bind_result_num) != 0)
	{
		...
		{
			if(this->enable_error_log)
			{
				SERVER_ERROR_LOG_stdlib_err("MOD MYSQL: Unable to allocate memory for mysql bind structs!");
			}
...
{
	this->enable_error_log = mysql_conn->error_log_enabled();
...
bool mysql_stmt_query::bind_param(unsigned int param_number,enum enum_field_types data_type,void* data,bool is_unsigned,unsigned long* len)
{
	if(param_number >= this->query_params_num)
	{
		if(this->enable_error_log)
		{
			SERVER_LOG_WRITE_ERROR.lock();
			SERVER_LOG_WRITE(SERVER_LOG_strtime(SERVER_LOG_LOCALTIME_REPORTING),true); 
			SERVER_LOG_WRITE(" MOD MYSQL: Unable to bind param to mysql stmt!\n",true);
			SERVER_LOG_WRITE("Param number out of range!\n\n",true);
			SERVER_LOG_WRITE_ERROR.unlock();
		}
...
bool mysql_stmt_query::error_log_enabled()
{
	return this->enable_error_log;
}

void mysql_stmt_query::enable_error_logging(bool enable)
{
	this->enable_error_log = enable;
}
