...
mysql_stmt_query(const void* mysql_conn);
void bind_param(unsigned int param_number,enum enum_field_types data_type,void* data,bool is_unsigned = false,unsigned long* len = NULL);

void bind_result(unsigned int result_number,enum enum_field_types data_type,void* data,bool* is_null = NULL,unsigned long* len = NULL,
unsigned long buffer_size = 0,bool* truncated = NULL);
